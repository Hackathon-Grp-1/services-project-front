  ...
  const formatMultipleQuestions = (text: string): ReactElement | null => {
    ...
    if (text.split("?").length - 1 >= 2) {
      ...
      if (firstQuestionPos > 0) {
        ...
        if (firstPart.split(" ").length > 5) {
          // If introduction is substantial
            ...
            {introText && (
              <Typography
                ...
                dangerouslySetInnerHTML={{
                  __html: convertMarkdownToHtml(introText),
                }}
                sx={markdownStyles}
              />
            )}
...
const ChatBox = ({
  ...
  useEffect(() => {
    ...
    if (initialPrompt && !initialPromptSent.current) {
      console.log(
        "Envoi automatique du message initial (première fois uniquement)..."
      );
      ...
      const sendFunction =
        chatType === "create_service" ? sendCreateServicePrompt : sendPrompt;
        ...
        .then((response) => {
          ...
          // Fallback pour l'ancien format
            ...
            if (typeof response.data[0].output === "string") {
              ...
              console.log(
                "Message extrait avec succès (ancien format string):",
                messageContent
              );
            } else if (
              response.data[0].output &&
              response.data[0].output.message
            ) {
              messageContent = response.data[0].output.message;
              console.log(
                "Message extrait avec succès (ancien format objet):",
                messageContent
              );
            }
          }
  ...
  const handleSendMessage = async () => {
    console.log(
      "handleSendMessage appelé avec message:",
      newMessage,
      "chatType:",
      chatType
    );
    ...
    if (newMessage === initialPrompt && initialPromptSent.current) {
      console.log(
        "Ce message initial a déjà été traité, abandon pour éviter doublon"
      );
      return;
    }
    ...
    if (sentMessages.current.has(messageKey.split("_")[0])) {
      console.log(
        "Message identique détecté, abandon pour éviter doublon:",
        messageKey
      );
      return;
    }
    ...
    try {
      ...
      const sendFunction =
        chatType === "create_service" ? sendCreateServicePrompt : sendPrompt;
      console.log(
        "Envoi du prompt au webhook avec type:",
        chatType,
        "message:",
        newMessage
      );
      ...
      // Fallback pour l'ancien format
        ...
        if (typeof response.data[0].output === "string") {
          ...
          console.warn(
            "Format de réponse spécifique non reconnu:",
            response.data[0]
          );
        }
      } else {
        ...
        >
          <AnimatePresence mode="sync">
            {messages.map((msg, index) => (
              <React.Fragment
                key={`container-${msg.timestamp.getTime()}-${index}`}
              >
                {/* Message container */}
                <Box
                  ...
                  sx={{
                    width: "100%",
                    mb:
                      msg.professionals && msg.professionals.length > 0 ? 1 : 2,
                    maxWidth: "900px",
                    display: "flex",
                ...
                >
                  <Box
                    sx={{
                      ...
                      margin: msg.isUser
                        ? "0 0.5rem 0 auto"
                        : "0 auto 0 0.5rem", // Meilleur espacement
